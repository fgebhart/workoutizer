...
def _get_latest_version_of(package: str):
    latest_version = str(
        subprocess.run([sys.executable, "-m", "pip", "install", f"{package}==random"], capture_output=True, text=True)
    )
    ...
    current_version = str(
        subprocess.run([sys.executable, "-m", "pip", "show", f"{package}"], capture_output=True, text=True)
    )
