...
from wizer.tests.helper import get_body


def test_help_view(db, client):
    response = client.get(reverse('help'))
    assert response.status_code == 200
    assert "Keyboard Navigation" in get_body(response)


def test_dashboard_view(db, client):
    response = client.get(reverse('home'))
    assert response.status_code == 200
    assert "Add Activity" in get_body(response)


def test_settings_view(db, client):
    response = client.get(reverse('settings'))
    assert response.status_code == 200
    assert "Save" in get_body(response)
