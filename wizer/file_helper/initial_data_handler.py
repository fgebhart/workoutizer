...
def change_date_of_demo_activities(activities: QuerySet):
    ...
    for i, activity in enumerate(activities.filter(is_demo_activity=True)):
        ...
        activity.date = today - datetime.timedelta(days=i * 3 + 1)
        activity.save()


def insert_custom_demo_activities(count: int, every_nth_day: int):
    pass
