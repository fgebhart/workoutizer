...
def test__insert_current_date_into_gpx(gpx_parser, test_data_dir):
    test_file = "template_example.gpx"
    path_to_output_dir = os.path.join(test_data_dir, "tested_data")
    date = datetime.datetime(2020, 4, 1, tzinfo=pytz.utc)
    _insert_current_date_into_gpx(
        gpx_file_name=test_file, source_path=test_data_dir, target_path=path_to_output_dir, strf_timestamp=date
    )
