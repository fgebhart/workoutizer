...
* Changed mounting mechanism to be triggered early on by `udev` whenever a Garmin device
  ...
  following:
  - In case your device gets mounted via `udev` and the new mounting mechanism, you no
    longer need to provide the `path_to_garmin_device` setting parameter, since the path
    gets detected automatically. Just leave the setting empty to disable the device
    watchdog in this case.
  - In case your device is mounted automatically on your system, you would still need to
    configure the path to your device within the setting page at `path_to_garmin_device`
    in order for workoutizer to regularly check for your device and new files.
